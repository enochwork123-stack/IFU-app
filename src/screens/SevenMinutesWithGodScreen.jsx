...
export function SevenMinutesWithGodScreen() {
  ...
  return (
    <>
      <PageHeader title={t("七分鐘與神獨處")} backTo="/journey/quiet-time" />
      ...
      <main className="px-6 pb-36 pt-8">
        ...
        <div className="mt-8 grid gap-6">
          <ArticleSection title="1. 晨更的緣起與挑戰">
            <QuoteBlock>{t("1882年，劍橋大學在其校園裏首次給世界興起這個口號：記得守晨更 (morning watch)。")}</QuoteBlock>
            <p>{t("一些像胡伯和東登的學生發覺他們的日子受到功課，講章，運動和競爭的壓力，生活被狂熱與活動佔有了。這一羣虔誠的信徒不久就看出他們的靈性受到威脅，若不及時援救將會達到不可收拾的地步。")}</p>
            <p>{t("他們在自我分析後所得的結論就是要守晨更：一個利用每個新的一天的首幾分鐘單終與神同在讀經禱告的計劃。" || "他們在自我分析後所得的結論就是要守晨更：一個利用每個新的一天的首幾分鐘單獨與神同在讀經禱告的計劃。")}</p>
            <QuoteBlock>{t("這個主意像點火燎原，原火燃燒的高潮在於七位劍橋的學生 (the Cambridge Seven 劍橋七傑) —— 特出的運動員及富有才華的學者，獻身做宣教，差傳的工作。他們丟下一切到中國為基督傳福音，這是一段值得注意的敎會興起的時期。")}</QuoteBlock>
            <p>{t("但這一羣人也同時發覺到，要能及時起床守晨更，其困難性與重要性互相媲美。其中的東登決心要把懶惰改成紀律。他設計出一個自動而有効醫治懶惰的秘方：他在床邊安置了一新奇的機械，設計「利用鬧鐘的震動力，發動釣魚竿上的線，把上了鈎的被單吊上半空，遠離睡者」。東登要起身會見他的上帝啊！")}</p>
          </ArticleSection>

          <ArticleSection title="2. 與神獨處的重要性">
            <p>{t("與上帝親密的交通，必需在每日晨更中重新得到。不論你如何稱呼：那安靜的時刻，個人靈修，晨更，或單獨崇拜 ……。這清晨的神聖幾分鐘解釋了基督教信仰的奧密。它有如一條聯繫被上帝使用的人的金錢，從摩西到大衛利賓士頓 (David Livingstone)；從先知阿摩司到葛培理，富有者與貧窮者，商人與軍人，任何被主重用的人都重視這段時間：與神獨處的時刻。")}</p>
            <p>{t("大衛在詩57:7說「上帝啊，我心堅定，我心堅定。」一個堅定的心能促成生命的穩定。很少信徒有這樣的心志和生命。其中所缺乏的就是一個「怎樣開始並維持晨更」的計劃。")}</p>
          </ArticleSection>

          <ArticleSection title="3. 開始你的七分鐘計劃">
            <p>{t("我建議你從七分鐘開始，五分鐘可能太短，十分鐘對初學者可能顯得太長。你願意抽出每早晨的七分鐘嗎？不是七天內的五個早晨，也不是六天裏的六個早晨，而是七天裏的七個早晨！ 祈求上帝幫助你：「上帝啊！我要在每早晨抽出最少七分鐘與你相會，明天鬧鐘在六點十五分响時，我要對你守約。」")}</p>
            <p>{t("或許你的禱告是這樣的：「耶和華啊！早晨祢必聽我的聲音，早晨我必向祢陳明我的心意，並要儆醒。」（詩5:3）你要怎樣利用這七分鐘呢？起床料理一番後，找一個肅靜的地方，預備好一本聖經，在那兒享受與神獨處的七分鐘 。")}</p>
            <h3 className="font-headline text-[1.3rem] text-primary">{t("第一階段：預備心（半分鐘）")}</h3>
            <p>{t("用前三十秒來預備你的心，感謝祂賜你好睡眠，也為新的一天稱謝。")}</p>
            <p>{t("禱告：「耶和華，潔淨我的心，用祢的道指教我，打開我的心，充滿我心，使我的頭腦清醒，我的靈魂活躍，我的心回應。耶和華啊，此刻以祢的同在圍繞我，阿門」。")}</p>
            <h3 className="font-headline text-[1.3rem] text-primary">{t("第二階段：讀經（四分鐘）")}</h3>
            <p>{t("現在用四分鐘讀經，你最大的需要是要聆聽上帝對你說的話，讓祂的道在你的心着火，會見它的作者！ 從福音書着手是比較適合的。從馬可福音開始，一節又一節，一章又一章地唸下去。勿貪快，但也避免鑽牛角尖。勿暫停的研究或查考聖經中的一些字句，思想或問題。讓這時的讀經成為一個暢讀的喜樂，並讓神向你說話。或許一次只唸廿節或一章，當你唸完馬可福音再向約翰福音下手。不久你會順利的唸完整個新約。")}</p>
            <h3 className="font-headline text-[1.3rem] text-primary">{t("第三階段：禱告（兩分半鐘）")}</h3>
            <p>{t("當神已藉着聖經向你說話後，你可藉着禱告向祂說話。你現在還有兩分鐘時間可與祂交通，禱告可順以下四個步驟進行：")}</p>
                ...
                <tbody>
                  {sevenMinuteRows.map(([activity, time]) => (
                    <tr key={activity} className="border-t border-outline-variant/45 first:border-t-0">
                      <td className="whitespace-pre-line px-4 py-4 text-primary">{t(activity)}</td>
                      <td className="w-24 px-4 py-4 font-extrabold text-secondary">{t(time)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <ArticleSection title="5. 結語與立約">
            <p>{t("七分鐘晨更法不是一成不變的規條，乃是一個指南。不久你會發記七分鐘的不足，逐漸你會增加到20分鐘，甚至30分鐘的寶貴時刻與上帝獨處，不要忠心於習慣而忠心於上帝。" || "七分鐘晨更法不是一成不變的規條，乃是一個指南。不久你會發覺七分鐘的不足，逐漸你會增加到20分鐘，甚至30分鐘 of 寶貴時刻與上帝獨處，不要忠心於習慣而忠心於上帝。")}</p>
            <p>{t("不要因别人如此行，你也照做。它不是無味的任務，也不是最終的目的，而是上帝已賜給這最寶貴的交通機會。現在就與神立約維護，培養並維持你七分鐘的晨更。")}</p>
          </ArticleSection>

          <ArticleSection title="The Cambridge Seven 劍橋七傑">
            <h3 className="font-headline text-[1.25rem] text-primary">{t("The Seven Students Who Became Missionaries to China")}</h3>
            <p>{t("The Cambridge Seven (劍橋七傑) were seven students from Cambridge University and one from the Royal Military Academy. In 1885, they gave up all wealth and privilege to become missionaries to China through the China Inland Mission (中國內地會 ; now known as The Overseas Missionary Fellowship 海外基督使團).  The seven were:")}</p>
