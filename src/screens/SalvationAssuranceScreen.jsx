      ...
      <div className="space-y-5 p-6">
        {section.scriptures?.map((scripture) => (
          <ScriptureCard
            ...
            reference={t(scripture.reference)}
            verse={t(scripture.verse || scripture.chinese)}
            tone={index === 1 ? 'glass' : 'light'}
          />
        ))}

        <div className="grid gap-3">
          {tContent(section, 'truths')?.map((truth, tIdx) => (
            <div
              key={tIdx}
    ...
    <>
      ...
      <main className="px-6 pb-36 pt-8 flex flex-col items-center">
        ...
        <section className="mt-8 w-full max-w-2xl">
          <JourneyPager next={{ to: '/journey/quiet-time', label: t('靈修') }} />
        </section>
      </main>
